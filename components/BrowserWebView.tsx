...
const BrowserWebView = observer(({ onMessage, onNavigationStateChange, injectedJavaScript, userAgent, style, ...otherProps }: BrowserWebViewProps) => {
  ...
  useEffect(() => {
    console.log('🔄 BrowserWebView: Updating refs for tabs:', tabStore.tabs.map(t => `${t.id}:${t.url}`))
  ...
  console.log('🔄 BrowserWebView: Rendering tabs:', tabStore.tabs.map(t => `${t.id}:${t.url} (active: ${t.id === tabStore.activeTabId})`))
